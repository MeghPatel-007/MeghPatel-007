...
from config import GITHUB_USERNAME, DISPLAY_NAME
from theme import (
    FONT_FAMILY, BG, BG2, FRAME, MUTED, INK, BORDER_RADIUS, TITLEBAR_H, PAD, DOT_COLORS,
    ASCII_ROW_DUR as ROW_DUR, ASCII_STAGGER as STAGGER
)
TITLE_TEXT = MUTED
CURSOR = INK
...
parts.append(f'<rect width="{CANVAS_W}" height="{CANVAS_H}" rx="{BORDER_RADIUS}" fill="url(#bg)"/>')
parts.append(f'<rect x="0.5" y="0.5" width="{CANVAS_W-1}" height="{CANVAS_H-1}" rx="{BORDER_RADIUS}" '
...
parts.append(f'<text x="{CANVAS_W/2}" y="{TITLEBAR_H/2 + 4}" fill="{TITLE_TEXT}" font-size="12" '
             f'text-anchor="middle">{GITHUB_USERNAME.lower()}@github:~$ ./portrait.sh</text>')
...
status_prefix = f"{GITHUB_USERNAME.lower()}@github:~$ whoami "
total_chars = len(status_prefix) + len(DISPLAY_NAME)
cursor_offset_x = round(total_chars * 5.7647)
parts.append(f'<text x="{PAD}" y="{status_y:.1f}" fill="{TITLE_TEXT}" font-size="13">'
             f'{status_prefix}<tspan fill="{INK}">{DISPLAY_NAME}</tspan></text>')
parts.append(f'<rect x="{PAD+cursor_offset_x}" y="{status_y-12:.1f}" width="8" height="14" fill="{INK}">'
