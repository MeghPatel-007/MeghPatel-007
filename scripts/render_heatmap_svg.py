...
from theme import (
    FONT_FAMILY, BG, BG2, FRAME, MUTED, INK, ACCENT, GREEN, GOLD,
    BORDER_RADIUS, TITLEBAR_H, PAD, DOT_COLORS,
    HEATMAP_COL_T as COL_T, HEATMAP_ROW_T as ROW_T, HEATMAP_CELL_DUR as CELL_DUR
)
TEXT = INK

HERE = os.path.dirname(__file__)
IN_PATH = os.path.join(HERE, "..", "data", "contributions.json")
OUT_PATH = os.path.join(HERE, "..", "heatmap.svg")
...
""".strip()
    ...
    parts = [
        ...
        '</defs>',
        f'<rect width="{canvas_w}" height="{canvas_h}" rx="{BORDER_RADIUS}" fill="url(#hbg)"/>',
        f'<rect x="0.5" y="0.5" width="{canvas_w-1}" height="{canvas_h-1}" rx="{BORDER_RADIUS}" '
    ...
    parts.append(f'<text x="{canvas_w/2}" y="{TITLEBAR_H/2 + 4}" fill="{MUTED}" font-size="12" '
                 f'text-anchor="middle">{GITHUB_USERNAME.lower()}@github: ~/contributions --graph</text>')
